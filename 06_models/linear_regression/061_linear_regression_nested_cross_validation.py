...
if __name__ == '__main__':
    ...
    for i in range(len(clust_matrix.columns)):
        ...
        for col, row in all_coeffs.iterrows():
            ...
            fisher_score = current_cluster[current_cluster['Feature'] == pred_feat]['FisherScore'].values[0]
            ...
            if ((csv['TargetFeature'] == clust_matrix.columns[i]) & (csv['Feature'] == pred_feat)).any():
                csv = csv.drop(csv[(csv['TargetFeature'] == clust_matrix.columns[i]) & (csv['Feature'] == pred_feat)].index)
