...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/2004856_TableS7.csv', header=3)
...
data = data[data['Position'].astype(str).str.contains(';') == False]
...
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if any(tp in col for tp in ['5min', '10min', '15min', '20min'])]
data = data[keepcols]
data

log_columns = [col for col in data.columns if any(tp in col for tp in ['5min', '10min', '15min', '20min'])]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/RCJD2014.csv', index=False)
