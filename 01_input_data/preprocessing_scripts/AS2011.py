...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/mcp.M110.004457-2.csv', header=0)
...
data = data[~data['Positions'].str.contains(';', na=False)]
...
data = data.drop(columns=[col for col in data.columns if 'Significance' in col or 
                          'Unmod. Pep.' in col or 'Localized' in col or 
                          'Nmods' in col or 'Variability' in col or 'Count' in col])
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/AS2011.csv', index=False)
