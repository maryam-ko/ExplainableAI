...
data = pd.read_csv('/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/161394_1_supp_584896_qhxn7d.csv', header=0)
...
data['Phosphosite'] = data['AA residue'].astype(str) + '(' + data['Position'].astype(str) + ')'
data

keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if col.startswith('Control') or col.startswith('anti-CD3') or col.startswith('anti-CD3+PDL2')]
data = data[keepcols]
data

Intensity_columns = [col for col in data.columns if col.startswith('Control') or col.startswith('anti-CD3') or col.startswith('anti-CD3+PDL2')]
...
def log2_transform(dataset):
    ...
    '''
    cols_to_transform = [col for col in data.columns if col.startswith('Control') or col.startswith('anti-CD3') or col.startswith('anti-CD3+PDL2')]
...
data.to_csv(f'/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/AST2020.csv', index=False)
