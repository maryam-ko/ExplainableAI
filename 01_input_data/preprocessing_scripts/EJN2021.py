...
data = pd.read_csv('/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/41587_2021_1099_MOESM3_ESM.csv', header=0)
...
# Filter out semi-colons from 'Amino acid', 'Positions', and 'Gene.names'
data = data[~data['Amino.acid'].str.contains(';', na=False)]
data = data[~data['Positions'].astype(str).str.contains(';', na=False)]
...
data['Phosphosite'] = data['Amino.acid'].astype(str) + '(' + data['Positions'].astype(str) + ')'
data

keepcols = ['Phosphosite', 'GeneName'] + [col for col in data.columns if col.startswith(('1_', '2_', '3_', '4_', '5_'))]
data = data[keepcols]
data

intensity_columns = [col for col in data.columns if col.startswith(('1_', '2_', '3_', '4_', '5_'))]
...
data.to_csv(f'/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/EJN2021.csv', index=False)
