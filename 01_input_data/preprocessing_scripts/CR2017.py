...
data['Phosphosite Probability'] = data['Phospho (STY) Probabilities'] \
    .str.findall(r'\(([\d.]+)\)') \
    .apply(lambda probs: list(map(float, probs)))

data = data[data['Phosphosite Probability'].apply(lambda x: any(p >= 0.85 for p in x))]
...
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if 'Ratio Replicate' in col]
data = data[keepcols]
data

Ratio_columns = [col for col in data.columns if 'Ratio Replicate' in col]
