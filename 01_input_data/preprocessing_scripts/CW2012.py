...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/1-s2.0-S1874391911005550-mmc5.csv', header=0)
print('Raw data loaded.')
data

data.columns = data.columns.str.strip()

data = data[data['Best Localized Phosphosite'].notna()]
data = data[data['Best Localized Phosphosite'].str.strip() != '']

data = data[data['Localization Probablity Exp. 1 Replicate A'] >= 0.85]

data['Amino acid'] = data['Best Localized Phosphosite'].str[0]         
data['Positions within proteins'] = data['Best Localized Phosphosite'].str[1:].astype(int)  
...
keepcols = ['Phosphosite'] + ['GeneName'] + [
    ...
    if ('Erl/control' in col or 'Gef/control' in col)
    and 'Replicate' in col
    and 'averaged' not in col
    and 'average of ≥2' not in col
]
data = data[keepcols]
data
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/CW2012.csv', index=False)
