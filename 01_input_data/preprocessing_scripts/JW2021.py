...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/BSR-2021-1326_suppSD1.csv', header=1)
...
data = data[~data['Best protein position'].astype(str).str.contains(';', na=False)]
...
data['Phosphosite'] = data['Amino acid'].astype(str) + '(' + data['Best protein position'].astype(str) + ')'

keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if col.startswith('Before_') or col.startswith('After_')]
data = data[keepcols]
data

Ratio_columns = [col for col in data.columns if col.startswith('Before_') or col.startswith('After_')]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/JW2021.csv', index=False)
