...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/41551_2020_585_MOESM4_ESM.csv', header=0)
...
data = data[data['N: Localization prob'] >= 0.85] 

# Filtering out semi-colons from 'Amino acid', 'Positions within proteins', and 'Gene names' columns
data = data[~data['C: Amino acid'].str.contains(';', na=False)]
data = data[~data['T: Positions within proteins'].astype(str).str.contains(';', na=False)]
data = data[~data['T: Gene names'].str.contains(';', na=False)]
data

# filter data
data['T: Sequence window'] = data['T: Sequence window'].str.replace('_', '')
data
...
data = match_seq_to_genename(data, 'T: Sequence window')

data['Phosphosite'] = data['C: Amino acid'].astype(str) + '(' + data['T: Positions within proteins'].astype(str) + ')'

keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if 'HCM_' in col or 'UA_' in col]
data = data[keepcols]
data

log2_columns = [col for col in data.columns if 'HCM_' in col or 'UA_' in col]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/UK2020.csv', index=False)
