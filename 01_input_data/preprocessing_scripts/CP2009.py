...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/Supplementary_Table_7.csv', header=1)
print('Raw data loaded.')
data

data.columns = data.columns.str.strip()
data = data.drop(index=[101, 102])
data['Localization Probability (p-value)'] = pd.to_numeric(
    data['Localization Probability (p-value)'], errors='coerce'
)

# filter data to keep only those with localization probability >= 0.85
data = data[data['Localization Probability (p-value)'] >= 0.85] 

# Filtering out semi-colons from 'Amino acid', 'Positions within proteins', and 'Gene names' columns
data = data[~data['Phosphorylated Amino Acid'].str.contains(';', na=False)]
data = data[~data['Phosphosite Position'].astype(str).str.contains(';', na=False)]
data = data[~data['Gene Names'].str.contains(';', na=False)]
data

# filter data
data['Sequence Window (13 amino acid sequence showing 6 amino acid flanking each side of phosphorylated site)'] = data['Sequence Window (13 amino acid sequence showing 6 amino acid flanking each side of phosphorylated site)'].str.replace('_', '')
data
...
data = match_seq_to_genename(data, 'Sequence Window (13 amino acid sequence showing 6 amino acid flanking each side of phosphorylated site)')

data['Phosphosite'] = data['Phosphorylated Amino Acid'].astype(str) + '(' + data['Phosphosite Position'].astype(str) + ')'

log2_columns = [
    'Ratio M/L',
    'Ratio M/L Normalized (5 nM dasatinib vs Ctrl)',
    'Ratio H/L',
    'Ratio H/L Normalized (50 nM dasatinib vs Ctrl)',
    'Ratio H/M',
    'Ratio H/M Normalized (50 nM dasatinib vs 5 nM dasatinib)'
]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/CP2009.csv', index=False)
