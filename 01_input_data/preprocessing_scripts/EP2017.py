...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/41598_2017_4798_MOESM3_ESM.csv', header=0)
print('Raw data loaded.')
data

data.columns = data.columns.str.strip()
data[['Amino acid', 'Positions within proteins']] = data['Phospho-site'].str.extract(r'([A-Z])(\d+)', expand=True)

# filter data to keep only those with localization probability >= 0.85
data = data[data['Site probability'] >= 85] 

data = data[~data['Amino acid'].str.contains(';', na=False)]
data = data[~data['Positions within proteins'].astype(str).str.contains(';', na=False)]
data = data[~data['Gene symbol'].str.contains(';', na=False)]
data

# filter data
data['Peptide sequence'] = data['Peptide sequence'].str.replace('_', '')
data
...
data = match_seq_to_genename(data, 'Peptide sequence')

data['Phosphosite'] = data['Amino acid'].astype(str) + '(' + data['Positions within proteins'].astype(str) + ')'

log2_columns = [col for col in data.columns 
             if 'log2 ratio' in col.lower() 
             and 'average' not in col.lower() 
             and 'adjusted' not in col.lower()]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/EP2017.csv', index=False)
