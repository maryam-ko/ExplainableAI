...
data = pd.read_csv('/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/Data Sheet 2.csv', header=0)
...
keepcols = ['Phosphosite', 'GeneName'] + \
           [col for col in data.columns if 'Ratio M/L normalized' in col] + \
           [col for col in data.columns if 'Ratio H/L normalized' in col] + \
           [col for col in data.columns if 'Ratio H/M normalized' in col] + \
           [col for col in data.columns if 'Ratio M/L normalized pP' in col] + \
           [col for col in data.columns if 'Ratio H/L normalized pP' in col] + \
           [col for col in data.columns if 'Ratio H/M normalized pP' in col]
...
Ratio_columns = list(dict.fromkeys(
    [col for col in data.columns if 'Ratio M/L' in col] + 
    [col for col in data.columns if 'Ratio H/L' in col] + 
    [col for col in data.columns if 'Ratio H/M' in col] + 
    [col for col in data.columns if 'Ratio M/L normalized' in col] + 
    [col for col in data.columns if 'Ratio H/L normalized' in col] + 
    [col for col in data.columns if 'Ratio H/M normalized' in col] +
    [col for col in data.columns if 'Ratio M/L pP' in col] +
    [col for col in data.columns if 'Ratio H/L pP' in col] +
    [col for col in data.columns if 'Ratio H/M pP' in col]
))
...
data.to_csv(f'/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/JJ2018.csv', index=False)
