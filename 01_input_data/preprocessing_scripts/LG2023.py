...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/mmc3 (2).csv', header=0)
print('Raw data loaded.')
data

# Extract amino acid and position using regex
data[['Amino acid', 'Position']] = data['Id-protein'].str.extract(r'_([A-Z])(\d+)', expand=True)
...
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if 'intensity' in col.lower()]
data = data[keepcols]
data

intensity_columns = [col for col in data.columns if 'intensity' in col.lower()]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/LG2023.csv', index=False)
