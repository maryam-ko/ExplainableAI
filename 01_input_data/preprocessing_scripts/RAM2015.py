...
data = pd.read_csv('/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/mcp.M114.046938-2.csv', header=0, skiprows=[1])
print('Raw data loaded.')
data

# filter data to keep only those with localization probability >= 0.85
data = data[data['Localiz. prob'] >= 0.85] 

# Filtering out semi-colons from 'Amino acid', 'Positions within proteins', and 'Gene names' columns
data = data[~data['Phos. Amino acid'].str.contains(';', na=False)]
data = data[~data['Position'].astype(str).str.contains(';', na=False)]
data = data[~data['Gene name'].str.contains(';', na=False)]
data

# filter data
data['13aa Sequence Window'] = data['13aa Sequence Window'].str.replace('_', '')
data
...
data = match_seq_to_genename(data, '13aa Sequence Window')

data['Phosphosite'] = data['Phos. Amino acid'].astype(str) + '(' + data['Position'].astype(str) + ')'

# Keep only 'Phosphosite' and ratio columns
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if 'Log2' in col]
data = data[keepcols]
data
...
data.to_csv(f'/Users/maryamkoddus/Documents/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/RAM2015.csv', index=False)
