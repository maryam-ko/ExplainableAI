...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/Supplementary table3.csv', header=2)
print('Raw data loaded.')
data

print(data.head())

# Remove the row containing replicate names, which is typically the first row of the data
...
data.rename(columns={
    ...
    'Control': 'Control Replicate 1', 
    'Unnamed: 7': 'Control Replicate 2', 
    'Unnamed: 8': 'Control Replicate 3',
    'CB treated': 'CB treated Replicate 1', 
    'Unnamed: 10': 'CB treated Replicate 2', 
    'Unnamed: 11': 'CB treated Replicate 3'
}, inplace=True)
print(data.columns)
...
data['Positions within proteins'] = data['Positions within proteins'].astype(str)
...
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if 'Replicate' in col]
data = data[keepcols]
data

log_columns = [col for col in data.columns if 'Replicate' in col]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/ZX2022.csv', index=False)
