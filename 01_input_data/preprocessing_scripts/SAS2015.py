...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/mcp.M114.047233-4.csv', header=2)
...
keepcols = ['Phosphosite', 'GeneName'] + [
    col for col in data.columns if col.startswith('AZD/DMSO') or col.startswith('PLX/DMSO') or col.startswith('PLX/AZD')
]
data = data[keepcols]
data

log2_columns = [col for col in data.columns if col.startswith('AZD/DMSO') or col.startswith('PLX/DMSO') or col.startswith('PLX/AZD')]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/SAS2015.csv', index=False)
