...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/mol212696-sup-0005-tables1.csv', header=0)
...
keepcols = ['Phosphosite'] + ['GeneName'] + [col for col in data.columns if col.startswith("Intensity") and "Ratio" not in col and "Occupancy" not in col]
data = data[keepcols]
data

Intensity_columns = [col for col in data.columns if col.startswith("Intensity") and "Ratio" not in col and "Occupancy" not in col]
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/AB2020.csv', index=False)
