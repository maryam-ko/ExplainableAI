...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/41467_2023_39514_MOESM5_ESM.csv', header=0)
print('Raw data loaded.')
data

data.columns = data.columns.str.strip()

data['Amino acid'] = data['ProteinPTMLocations'].str.extract(r'\((\D)')  # Extract letter
data['Positions within proteins'] = data['ProteinPTMLocations'].str.extract(r'(\d+)\)') # Extract digits
...
data = data[~data['GeneName'].str.contains(';', na=False)]
data

# filter data
data['Seqwindows'] = data['Seqwindows'].str.replace('_', '')
data
...
data = match_seq_to_genename(data, 'Seqwindows')
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/WZ2023.csv', index=False)
