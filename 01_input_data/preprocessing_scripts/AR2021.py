...
data = pd.read_csv('/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/raw_data/41420_2021_456_MOESM8_ESM.csv', header=0)
...
data[['Amino acid', 'Positions within proteins']] = data['Phosphorylated residue'].str.extract(r'([A-Z])(\d+)', expand=True)
...
log2_cols = ['L41 1 μM', 'L41 10 μM', 'ALG 1 μM', 'ALG 10 μM', 'KD 48h', 'KD 72h']
keepcols = ['Phosphosite'] + ['GeneName'] + log2_cols
data = data[keepcols]
data

data[log2_cols] = data[log2_cols].apply(pd.to_numeric, errors='coerce')
...
def log2_transform(dataset):
    ...
    '''
    cols_to_transform = dataset[log2_cols]
    dataset[log2_cols] = cols_to_transform.apply(np.log2)
...
data.to_csv(f'/data/home/bt24990/maryam-ko-QMUL-MSc-Project/01_input_data/PreprocessedDatasets/AR2021.csv', index=False)
