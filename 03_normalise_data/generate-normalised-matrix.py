...
total_rows = len(dataset_list)
print("Total rows (datasets):", total_rows)
unique_columns = set()
for df in dataset_list:
    unique_columns.update(df.columns)

total_columns = len(unique_columns)
print("Total unique phosphosites (columns):", total_columns)
...
all_names = (# AB2020_names + 
            ...
            BS2014_names + CF2017_names + CK2022_names + CKC2017_names + CP2009_names + 
            CR2017_names + CW2012_names + DB2022_names + DL2023_names + EJN2021_names + 
            EP2017_names + FR2021_names + FSO2012_names + FSO2013_names + FS2019_names + 
            GF2025_names + GRW2016_names + HH2022_names + HS2024_names + HW2015_names + 
            # IUA2019_names + 
            IMM2018_names + JAW2011_names + # JB2023_names +
            JC2019_names + JJ2018_names + JVO2010_names + JW2015_names + JW2021_names + 
            KBE2022_names + 
            # KDH2022_names + 
            KS2014_names + KTGR2014_names + LATM2013_names + LG2023_names + LY2019_names +
            MM2018_names + MV2014_names + NJH2015_names + NJH2024_names + NJH2025_names + 
            PG2020_names + RAM2015_names + RB2022_names + RBK2021_names + RCJD2014_names + 
            RKK2015_names + RM2009_names + RM2024_names + RN2012_names + RNJ2017_names + 
            SAS2015_names + SFR2015_names + SR2019_names + TM2022_names + UK2020_names + 
            VOI2024_names + WZ2023_names + YA2020_names + YB2013_names + YC2018_names +
            YP2020_names + ZCP2016_names + # ZM2022_names + 
            ZQ2022_names + ZX2022_names)

# Get the order of dataset names returned by your function
grouped_dataset_names = [df.index[0] for df in dataset_list]

print(grouped_dataset_names)  # print first 10 dataset names returned by your function
print(all_names)  # print first 10 dataset names from your all_names list
...
temp_dict = {df.index[0]: df for df in dataset_list}

# Filter out dataset names not present in temp_dict
filtered_names = [name for name in all_names if name in temp_dict]

# Build dataset_dict only for datasets that exist
dataset_dict = {name: temp_dict[name] for name in filtered_names}

# log which dataset names were missing
missing_names = [name for name in all_names if name not in temp_dict]
if missing_names:
    print(f"These datasets were missing and will be skipped: {missing_names}")

print(dataset_dict['AH2018_TMTMS2_1_C'].head())

print(df.dtypes.unique())
print(df.isna().sum().sum())  # total NaNs in DataFrame
rows, columns = df.shape
print(f"Rows: {rows}, Columns: {columns}")
for dataset_name, df in dataset_dict.items():
    rows, cols = df.shape
    print(f"Dataset '{dataset_name}': Rows = {rows}, Columns = {cols}")
for dataset_name, df in dataset_dict.items():
    numeric_cols = df.select_dtypes(include='number').columns
    print(f"Dataset '{dataset_name}': Rows = {df.shape[0]}, Numeric columns = {len(numeric_cols)}")
...
print (norm_matrix.columns.name)
# get unique phosphosites
uniq_phos = norm_matrix.columns.drop('DatasetName').unique()
print(f'Unique phosphosites:', uniq_phos)
print(f'Number of unique phosphosites:', len(uniq_phos))
