...
def create_submatrix_from_clustered_matrix(prots_to_keep, subset_name, min_vals):
    ...
    """
    full_matrix = pd.read_csv(f'/data/home/bt24990/ExplainableAI/04_clustering/interim_data/ClusteredMatrix_Min{min_vals}Vals.csv', header=0, index_col=0)
    ...
    submatrix.to_csv(f'/data/home/bt24990/ExplainableAI/04_clustering/interim_data/ClusteredMatrix_{subset_name}_Min{min_vals}Vals.csv', index=False)
    return submatrix
...
#     clusters_output.to_csv(f'/data/home/bt24990/ExplainableAI/04_clustering/interim_data/{filename}.csv', index=True)
...
def calculate_optimal_clusters(matrix, filename):
    ...
    prefix = all_sites[0].split('_')[0]

    # Validate
    if len(all_sites) <= 2:
        raise ValueError(f"{prefix} has <=2 phosphosites — cannot cluster meaningfully.")

    # Fill missing values
    new_df = matrix[all_sites]
    new_df = new_df.apply(lambda x: x.fillna(x.mean()), axis=0).fillna(0)
    new_df = new_df.T.drop_duplicates()

    row_data = {'Prefix': prefix}

    # Clustering over 100 random seeds
    for random_seed in random_seed_list:
        silhouette_avg = []
        for i in range(2, min(len(new_df), 20)):  # cap clusters to avoid overfitting
        ...
        if silhouette_avg and max(silhouette_avg) > -1:
            ...
            print(f'No valid clustering for {prefix}, defaulting to 1 cluster.')
            optimal_k = 1

        row_data[f'RandomSeed:{random_seed}'] = optimal_k

    # Build results
    clusters_df = pd.DataFrame([row_data])
    clusters_df.set_index('Prefix', inplace=True)

    # Mode or mean of optimal k values
    optimal_clusters = clusters_df.mode(axis=1)
    if optimal_clusters.shape[1] > 1:
        optimal_k = pd.DataFrame(optimal_clusters.mean(axis=1)).astype(int)
    else:
        optimal_k = optimal_clusters
    ...
    clusters_output.to_csv(os.path.join(interim_dir, f'{filename}.csv'), index=True)
...
#     output_matrix.to_csv(f'/data/home/bt24990/ExplainableAI/04_clustering/interim_data/{filename}.csv', index=False)
...
def create_clustered_matrix_from_normalised_matrix(normalised_matrix, optimal_clusters, filename):
    ...
    for ppsite in normalised_matrix.columns:
        ...
        if prefix not in prefixes:
            prefixes[prefix] = [ppsite]
        else:
            prefixes[prefix].append(ppsite)
    ...
    # Only process the first protein in the matrix
    first_prefix = next(iter(prefixes.items()))
    prefix, sites = first_prefix

    if len(sites) == 1:
        pass  # Only one site, skip
    elif len(sites) == 2:
        ...
        ppsites_df = new_df.groupby('cluster').mean(numeric_only=True)
        ppsites_df = ppsites_df.sort_values(by='cluster')
