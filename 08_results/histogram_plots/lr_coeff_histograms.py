...
def plot_LR_coeff_histograms_across_thresholds(base_dir, thresholds):
    ...
    for threshold in thresholds:
        ...
        plt.savefig(f"{base_dir}/08_results/plots/LR_MedianCoeff_Histogram_Min{threshold}Vals.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_LR_CoeffxR2_histograms_across_thresholds(base_dir, thresholds):
    ...
    for threshold in thresholds:
        ...
        plt.grid(axis='y', linestyle='-', alpha=0.3)
        plt.savefig(f"{base_dir}/08_results/plots/LR_CoeffxR2_Histogram_Min{threshold}Vals.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def combine_histogram_pngs_grid(base_dir, thresholds):
    ...
    for i, threshold in enumerate(thresholds):
        # Top row: MedianCoeff
        img1_path = f"{base_dir}/08_results/plots/LR_MedianCoeff_Histogram_Min{threshold}Vals.png"
        img1 = mpimg.imread(img1_path)
        axs[0, i].imshow(img1)
        axs[0, i].axis('off')
        # Bottom row: Coeff*R2
        img2_path = f"{base_dir}/08_results/plots/LR_CoeffxR2_Histogram_Min{threshold}Vals.png"
        img2 = mpimg.imread(img2_path)
        axs[1, i].imshow(img2)
        axs[1, i].axis('off')
    plt.tight_layout()
    plt.savefig(f"{base_dir}/08_results/plots/LR_Histograms_Grid.png", dpi=300, bbox_inches='tight')
    plt.close()
