...
def plot_XGB_SHAP_histograms_across_thresholds(base_dir, thresholds):
    ...
    for threshold in thresholds:
        ...
        axs = plt.gca()
        axs.xaxis.set_major_formatter(plt.FormatStrFormatter('%.3f'))
        # Set x-ticks from -0.005 to 0.005 in steps of 0.005
        min_tick = np.floor(shap_vals.min() / 0.005) * 0.005
        max_tick = np.ceil(shap_vals.max() / 0.005) * 0.005
        xticks = np.arange(min_tick, max_tick + 0.005, 0.005)
        axs.set_xticks(xticks)
        plt.savefig(f"{base_dir}/08_results/plots/XGB_SHAPValue_Histogram_Min{threshold}Vals.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_XGB_SHAPxR2_histograms_across_thresholds(base_dir, thresholds):
    ...
    for threshold in thresholds:
        ...
        axs.xaxis.set_major_formatter(plt.FormatStrFormatter('%.3f'))
        # Set x-ticks from -0.005 to 0.005 in steps of 0.005
        min_tick = np.floor(shap_r2_vals.min() / 0.005) * 0.005
        max_tick = np.ceil(shap_r2_vals.max() / 0.005) * 0.005
        xticks = np.arange(min_tick, max_tick + 0.005, 0.005)
        axs.set_xticks(xticks)
        plt.savefig(f"{base_dir}/08_results/plots/XGB_SHAPxR2_Histogram_Min{threshold}Vals.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def combine_xgb_histogram_pngs_grid(base_dir, thresholds):
    ...
    for i, threshold in enumerate(thresholds):
        # Top row: SHAPValue
        img1_path = f"{base_dir}/08_results/plots/XGB_SHAPValue_Histogram_Min{threshold}Vals.png"
        img1 = mpimg.imread(img1_path)
        axs[0, i].imshow(img1)
        axs[0, i].axis('off')
        # Bottom row: SHAP*R2
        img2_path = f"{base_dir}/08_results/plots/XGB_SHAPxR2_Histogram_Min{threshold}Vals.png"
        img2 = mpimg.imread(img2_path)
        axs[1, i].imshow(img2)
        axs[1, i].axis('off')
    plt.tight_layout()
    plt.savefig(f"{base_dir}/08_results/plots/XGB_Histograms_Grid.png", dpi=300, bbox_inches='tight')
    plt.close()
